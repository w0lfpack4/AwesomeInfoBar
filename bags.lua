...
AIB.plugins["Bags"] = {
	...
	UpdateBags = function()
		...
		if (AIB.saved.account.Bags.on and not snoozing) then
			...
			-- set value
			if (AIB.saved.account.Bags.onlyShowFreeSpace) then
				value = freeInv
			else
				value = curInv .. " / " .. maxInv
			end			
...
AIB.plugins.Bags.Menu = {	
	{
		...
		controls = {
			...
			{
				type = "checkbox",
				name = "Only show free space", 
				tooltip = "If checked, displays only free bag space. If not checked, displays used/total bag space.",
				getFunc = function() return AIB.saved.account.Bags.onlyShowFreeSpace end,
				setFunc = function(newValue) AIB.saved.account.Bags.onlyShowFreeSpace = newValue; AIB.plugins.Bags.UpdateBags() end,
				disabled = function() return not(AIB.saved.account.Bags.on) end,
				default = AIB.defaults.Bags.onlyShowFreeSpace,			
			},
			{
				type = "checkbox",
